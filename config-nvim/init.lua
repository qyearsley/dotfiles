...
require("lazy").setup({
  ...
  -- cmp-nvim-lsp is a dep here so capabilities are ready when LSP attaches
  {
    ...
    config = function()
      require("mason").setup()
      -- automatic_enable defaults to true, so mason-lspconfig enables each
      -- installed server itself. The vim.lsp.config calls below only supply
      -- the settings it looks up when one attaches.
      ...
      local on_attach = function(client, bufnr)
        ...
        -- jump() rather than the goto_prev/goto_next pair, which Neovim removes
        -- in 0.13. No float: virtual_text already prints the diagnostic on the
        -- line you land on.
  ...
  -- Autocompletion engine with LSP integration
  {
    ...
    dependencies = {
      "hrsh7th/cmp-buffer",
      "hrsh7th/cmp-path",
      -- Also a dependency of nvim-lspconfig above, and lazy dedupes it. Named
      -- here too because the `nvim_lsp` source below needs it: relying on
      -- lspconfig's BufReadPre firing before this plugin's InsertEnter holds
      -- for a file argument, but not for a buffer that was never read.
      "hrsh7th/cmp-nvim-lsp",
    },
    config = function()
